...
def find_beautiful_languages(languages)
  ...
  if info[:is_beautiful?]
    beautiful_language[language] = info
    end
  end
return beautiful_language
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  #                 is_beautiful?: false }
  # }
  language_key = language_name.to_sym

  if languages[language_key] && languages[language_key][:facts]
    languages[language_key][:facts][fact_index]
  else
    nil
  end
end
